...
func Movie(card, ticket int, perc float64) int {
	count := 0
	lastPayment := float64(ticket)
	systemB := float64(card) + lastPayment
	for math.Ceil(systemB) >= float64(ticket*(count+1)) {
		lastPayment *= perc
		systemB += lastPayment
		count++
	}
...
func Movie1(card, ticket int, perc float64) (count int) {
	var systemA float64
